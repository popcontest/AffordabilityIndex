...
def build_readme(provenance: pd.DataFrame, splice_note: str, spx_source: str, window: int) -> pd.DataFrame:
    ...
    entries = [
        ...
        ("FINDING heterodox data", "Twelve non-standard series were scored. The long-history ones "
                                   "mostly fail as leading indicators: electricity output 0.69x, M2 "
                                   "velocity 0.21x, consumer sentiment 1.07x, policy uncertainty "
                                   "0.13x, durable goods spending 0.57x. Durables produce the widest "
                                   "coincident/leading split in the entire model -- 8.32x against "
                                   "0.57x -- big-ticket purchases collapse IN a downturn and say "
                                   "nothing before it. The short-history ones look better (temp help "
                                   "2.33x, rail carloads 1.78x, truck tonnage 1.42x) but each covers "
                                   "only two or three recessions and fires in fewer than ten distinct "
                                   "episodes, so the numbers are not evidence."),
        ("FINDING short history is fatal", "The obvious rescue for short-history data is to point it "
                                           "at a frequent target instead of recessions. It was tested "
                                           "and it does NOT work. A block of 2000s-onward series "
                                           "(quits, rail, truck, temp help, business applications) "
                                           "scores AUC 0.242 on Fed hikes and 0.378 on Fed cuts -- "
                                           "both BELOW 0.5, meaning the fitted relationships inverted "
                                           "out of sample -- against the policy block's 0.847 on the "
                                           "same target. On recessions it produced 74 out-of-sample "
                                           "months containing ZERO events. The reason is that "
                                           "walk-forward validation consumes history before it can "
                                           "emit a single honest prediction: a frequent target buys "
                                           "more events per month, not more months. History length, "
                                           "not event frequency, is the binding constraint. The "
                                           "practical bar for a new series here is roughly a 1970 "
                                           "start."),
        ("NOTE alternative data", "Google Trends, card spend, job postings and similar shine at "
                                  "NOWCASTING a continuous variable this quarter, where a decade of "
                                  "history supplies hundreds of usable observations. This model "
                                  "forecasts rare events twelve months out, which is the one problem "
                                  "shape their history cannot support. That is a mismatch of problem "
                                  "to data, not a defect in either."),
